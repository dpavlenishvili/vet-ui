...
export class SchedulesService {
  ...
  /**
   * @description Set pass level * * @tags Schedules
   * @name SchedulesPassLevel
   * @summary Set pass level if selection category_id is 2
   * @request POST:/schedules/{id}/pass-level/{passLevel} */

  schedulesPassLevel = (id: string, passLevel: boolean, options: HttpRequestOptions = {}) => {
    return this.httpClient.post<{
      data?: {
        /** Indicates if the operation was successful */
        status?: boolean;
      };
    }>(`${this.baseUrl}/schedules/${id}/pass-level/${passLevel}`, null, options);
  };
}
