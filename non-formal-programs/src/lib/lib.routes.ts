...
export const nonFormalProgramsRoutes: Route[] = [
  {
    path: '',
    loadComponent: () =>
      import('./non-formal-programs.component')
        .then((m) => m.NonFormalProgramsComponent),
    pathMatch: 'full',
  ...
  },
  {
    path: ':programId',
    loadComponent: () =>
      import('./non-formal-program-page/non-formal-program-page.component')
        .then((m) => m.NonFormalProgramPageComponent),
    canActivate: [unAuthenticatedGuard],
    data: breadcrumb([
      ...baseBreadcrumbItems,
      { path: '/programs/non-formal', text: 'non_formal.programs' },
